...
/**
 * 工具调用事件 - 类似 LlamaIndex 的 ToolCall
 */
...
/**
 * 工具调用结果事件 - 类似 LlamaIndex 的 ToolCallResult
 */
...
export const WorkflowEvents = {
  ...
  // Agent 流式事件
  agentStream: createWorkflowEvent<AgentStreamData>("agent:stream"),
  agentOutput: createWorkflowEvent<AgentOutputData>("agent:output"),

  // 工具调用事件
  toolCall: createWorkflowEvent<ToolCallInfo>("tool:call"),
  toolCallResult: createWorkflowEvent<ToolCallResultData>("tool:result"),
  ...
  // 跟进上下文事件
  followUpContextSet: createWorkflowEvent<{ originalRequest: string; suggestedAction: string }>(
...
export class WorkflowContext {
  ...
  // v2.9.51: 统一事件结构，使用 WorkflowEvent 的 data 字段
  ...
  /**
   * 消费下一个事件
   * v2.9.51: 返回统一的 WorkflowEvent 格式
   */
  ...
  /**
   * 获取工作流状态 (LlamaIndex: context.state)
   */
  ...
  /**
   * 更新状态 (LlamaIndex: context.state.xxx = value)
   */
...
export class WorkflowEventRegistry {
  ...
  /**
   * 注册事件处理器 (LlamaIndex: workflow.handle([EventType], handler))
   */
...
/**
 * v2.9.47: 工作流事件流 (借鉴 LlamaIndex stream.until().toArray())
 *
 * 支持流式处理工作流事件，直到满足停止条件
 */
export class WorkflowEventStream {
  ...
  /**
   * 设置停止条件 (LlamaIndex: stream.until(stopEvent))
   */
  ...
  /**
   * 将流转换为数组 (LlamaIndex: stream.toArray())
   */
...
/**
 * v2.9.47: 创建工作流执行器 (简化版 LlamaIndex createWorkflow)
 *
 * 用法:
 * ```typescript
 * const workflow = createSimpleWorkflow()
 *   .on("taskStart", async (ctx, data) => { ... })
 *   .on("toolCall", async (ctx, data) => { ... });
 *
...
export function createSimpleWorkflow(): SimpleWorkflow {
  ...
  const workflow: SimpleWorkflow = {
     ...
     * 运行工作流
     */
    async run(context: WorkflowContext): Promise<WorkflowEventStream> {
      while (!context.isCancelled) {
        const event = context.consumeEvent();
        if (!event) break;

        stream.push(event);
        // WorkflowEvent 使用 data 属性
