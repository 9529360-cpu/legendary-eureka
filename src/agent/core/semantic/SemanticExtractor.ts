...
import { SemanticExtraction, IntentType, ExtractedEntities, ExtractedConstraints } from "../types";
 ...
 * 范围模式
 */
const RANGE_PATTERNS = [/([A-Z]+\d+:[A-Z]+\d+)/gi, /([A-Z]+:\s*[A-Z]+)/gi, /(\d+:\d+)/g];
...
export class SemanticExtractor {
  ...
  extract(userInput: string, _context?: Record<string, unknown>): SemanticExtraction {
    ...
    const _compressedIntent = compressToSuperIntent(semanticAtoms);
