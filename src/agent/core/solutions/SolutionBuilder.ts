...
const SOLUTION_TEMPLATES: SolutionTemplate[] = [
  // 创建公式
  {
    ...
    recommended: {
      ...
      steps: ["选中数据区域", '定义命名范围（如 "销售数据"）', "使用 =SUM(销售数据) 代替绝对引用"],
      code: "=SUM(销售数据)",
  ...
  // 格式化
  {
    ...
    recommended: {
      ...
      steps: ["选中数据区域", "开始 → 条件格式化 → 新建规则", "设置条件和格式"],
    ...
    structural: {
      ...
      steps: ["定义统一的单元格样式", "创建自定义格式模板", "使用格式刷批量应用"],
  ...
  // 数据清洗
  {
    ...
    recommended: {
      ...
      steps: ["对输入列设置数据验证", "使用 ARRAYFORMULA 自动清洗新数据", "建立错误数据提示"],
      pros: ["防患于未然", "自动化处理"],
      cons: ["需要设置验证规则"],
    },
    structural: {
      ...
      steps: ["设计数据输入表单", "创建数据清洗中间层", "使用 Power Query 自动化处理"],
  ...
  // 数据分析
  {
    ...
    recommended: {
      ...
      title: "推荐方案",
      description: "使用数据透视表",
      steps: ["选中数据区域", "插入 → 数据透视表", "配置行/列/值字段"],
    ...
    structural: {
      ...
      steps: ["创建独立的分析工作表", "使用切片器控制透视表", "添加图表可视化"],
      pros: ["专业仪表板", "一目了然", "可分享"],
      cons: ["需要设计和规划", "维护成本"],
    },
  },
...
export class SolutionBuilder {
  ...
  buildFromDiagnosis(diagnosis: DiagnosticResult): LayeredSolution {
    ...
    return {
      ...
      structural: {
        ...
        steps: ["审视当前数据架构", "考虑是否需要重构表结构", "建立数据验证机制"],
        pros: ["长期收益", "系统性解决"],
   ...
   * 定制化选项
   */
  private customizeOption(option: SolutionOption, extraction: SemanticExtraction): SolutionOption {
    ...
    if (extraction.constraints.preserveFormat && customized.steps) {
      customized.steps = ["备份原有格式", ...customized.steps, "验证格式保持一致"];
    }
